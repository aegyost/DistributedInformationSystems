	...
	for l := 1; l <= maxLength; l++ {
		...
		for i := 0; i < l; i++ {
			if power > (^uint64(0))/base {
				return 0, fmt.Errorf("переполнение")
			}
			power *= base
		}
...
func WordByIndex(index uint64, alphabet string, maxLength int) (string, error) {
	base := uint64(len(alphabet))
	if base == 0 {
		return "", fmt.Errorf("пустой алфавит")
	}
	remaining := index
	for l := 1; l <= maxLength; l++ {
		count := uint64(1)
		for i := 0; i < l; i++ {
			if count > (^uint64(0))/base {
				return "", fmt.Errorf("переполнение")
			}
			count *= base
		}
		if remaining < count {
	...
	return "", fmt.Errorf("индекс слова %d выходит за пределы допустимых", index)
}
